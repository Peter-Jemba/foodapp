...
from models.db_config import DBConn

db = DBConn()

secrete_key = '^G7JYT@XQP"*19$&KWL'
...
def create_user():
    db.cursor.execute("INSERT INTO users(name, email, password, is_admin) VALUES('%s', '%s', '%s', 'true');")
    user = db.cursor.append(user)
    return user
